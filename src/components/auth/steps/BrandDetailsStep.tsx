...
export function BrandDetailsStep({ initialData, onNext }: BrandDetailsStepProps) {
  const {
    register,
    handleSubmit,
    setValue,
    ...
    defaultValues: {
      ...
      countryCode: "+91", // Default matching Figma
    },
  })

  const onSubmit = (data: BrandDetailsFormValues) => {
    onNext({
    ...
    <div className="w-full bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] p-6 sm:p-8">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="brandName" className="text-xs text-text-light ml-1">Brand Name</Label>
          <Input 
            id="brandName" 
        ...
        <div className="space-y-1.5">
          <Label htmlFor="phone" className="text-xs text-text-light ml-1">Phone Number</Label>
          <div className="flex gap-2">
            <div className="w-[100px]">
              <Select defaultValue="+91" onValueChange={(val) => setValue("countryCode", val)}>
