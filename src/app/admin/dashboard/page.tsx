...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (user !== "harikishorereddy9908@gmail.com") {
      ...
      if (typeof window !== "undefined") {
         const subs = JSON.parse(localStorage.getItem("geonixa_submissions") || "{}");
         setSubmissions(subs);
         const logins = JSON.parse(localStorage.getItem("geonixa_valid_logins") || "{}");
         setRegisteredUsers(logins);
      }
    }
  }, []);
  ...
  const handleDeleteTarget = (targetEmail: string) => {
               ...
               <button 
                  onClick={() => {
                     if(confirm("CRITICAL WARNING: Are you sure you want to completely erase ALL student profiles, logins, and exam submissions? This action cannot be undone.")) {
                        localStorage.removeItem("geonixa_submissions");
                        localStorage.removeItem("geonixa_valid_logins");
                        localStorage.removeItem("geonixa_student_profiles");
         ...
         {completedEmails.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
               {completedEmails.map((email, idx) => {
                  ...
                  const profileDataStr = typeof window !== 'undefined' ? localStorage.getItem("geonixa_student_profiles") : null;
                  const profilesObj = profileDataStr ? JSON.parse(profileDataStr) : {};
                  const profile = profilesObj[email] || {};
                     ...
                     <div key={email} className={`student-card ${printingEmail && printingEmail !== email ? 'no-print' : ''}`} style={{ backgroundColor: "var(--card-bg)", borderRadius: "12px", border: "1px solid var(--border-color)", padding: "2rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                        ...
                        <div style={{ display: "flex", gap: "2rem" }}>
                           ...
                           <div style={{ flex: 2 }}>
                              ...
                              <div className="score-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1px", backgroundColor: "var(--border-color)", border: "1px solid var(--border-color)", borderRadius: "8px", overflow: "hidden" }}>
