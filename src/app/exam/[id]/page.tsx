  ...
  const handleFinalSubmit = useCallback(async (reason: string = "MANUAL_SUBMIT") => {
    ...
    try {
      ...
      roundResults.push({
        ...
        totalMarks: 60,
        percentage: (Math.max(0, r4Score) / 60) * 100,
        startTime: examStartTime,
        endTime: submissionTimestamp,
      ...
      const examResult: ExamResult = {
        ...
        examPattern,
        totalScore,
        qualificationStatus: totalScore >= 90 ? 'QUALIFIED' : (reason === "TERMINATED" ? 'TERMINATED' : 'DISQUALIFIED'),
              ...
              {isMcqDomain ? (
                <div style={{ maxWidth: "1500px", margin: "0 auto", width: "100%", display: "flex", flexDirection: "column" }}>
                  ...
                  <div style={{ padding: "1.5rem", backgroundColor: "white", borderRadius: "16px", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.05)", border: "1px solid var(--border-dim)", flex: 1, display: "flex", flexDirection: "column" }}>
                    ...
                    <div style={{ display: "grid", gap: "0.6rem" }}>
                      {(() => {
                        const currentQ = codingQuestions[codingQuestionIndex];
                        if (!currentQ) return null;
                        ...
                        return optionsArray.map((opt: any, optionIndex: number) => {
                          const optText = typeof opt === 'object' && opt !== null ? (opt.text || opt.value || opt.label || JSON.stringify(opt)) : String(opt);
                          const isSelected = codingAnswers[codingQuestionIndex] === optText || codingAnswers[codingQuestionIndex] === opt;
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
                  ...
                  <div style={{ flex: 1, display: "flex", gap: "1rem", minHeight: 0 }}>
                    <div style={{ flex: "0 0 40%", padding: "1.5rem", backgroundColor: "whitesmoke", color: "#0f172a", borderRadius: "16px", border: "1px solid var(--border-dim)", overflowY: "auto", boxShadow: "0 4px 6px -1px rgba(0, 0, 0, 0.05)" }}>
